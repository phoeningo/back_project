...
def set_volume_cpu(Bigbox,little_volume,cx,cy,cz,radius):
  try:
    x,y,z=little_volume.shape
    for i in range(x):
      for j in range(y):
        for k in range(z):
          Bigbox[cx-radius+i,cy-radius+j,cz-radius+k]+=little_volume[i,j,k]  
...
def read_dynamo_transform(input_file):
  ...
  for linei in range(L-1):
    ...
    if (len(line_split)>1):
      ...
      tomo_number=line_remove[19]
      if tomo_number not in tomoname:
        tomoname.append(tomo_number)
        tomos.append([[],[]])
  ...
  for linei in range(L-1):
    ...
    if (len(line_split)>1):
      line_remove=Remove(line_split,'')
      #tmp_euler.
      tomos[tomoname.index(line_remove[19])][0].append([ float(line_remove[6])/180*Pi,float(line_remove[7])/180*Pi,  float(line_remove[8])/180*Pi  ])
      #tmp_coords.
      tomos[tomoname.index(line_remove[19])][1].append(  [ ( float(line_remove[3])+float(line_remove[23]) ) /2 , (float(line_remove[4])+float(line_remove[24]) )/2, (float(line_remove[5])+float(line_remove[25]))/2 ]   )
  return tomos  
 # return tmp_euler,np.array(tmp_coords)


def write_one_tomo(tomo,count):
  euler=tomo[0]
  coords=np.array(tomo[1])
 # print(coords)
  X=coords[:,0]
  Y=coords[:,1]
  Z=coords[:,2]
...
#so ,transform vector is -1*(start_x,start_y,start_z),
  output_name='t'+str(count)+args.output_mrc
  try:
    output_full_volume=M.read_float_mrc(output_name)
  except:
    output_full_volume=np.zeros([out_box_size,out_box_size,out_box_size],dtype=ori_volume.dtype)
  print(output_full_volume.shape)
#print (X)
#print('Init...')
  ...
  #print(R)
    ...
    set_volume_cpu(output_full_volume,output_volume,cdx,cdy,cdz,radius)

 # set_volume[t_bd,t_gd](output_full_volume,output_volume,cdx,cdy,cdz,radius)
    print('Writing ...'+str(vi+1)+'/ '+str(len(coords))+' subvolume')
  #M.write_pix_file(output_volume,'sub'+str(vi)+'.mrc',1)

  print('Prepare tomo'+str(count))
  M.write_pix_file(output_full_volume,output_name,args.angpix)
  print(time.time()-t1)
...
#euler,coords
tomos=read_dynamo_transform(args.input_table)
